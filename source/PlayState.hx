...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			var bg:BGSprite = new BGSprite('madbuns_street', -270, -300, 0.9, 0.9);
            bg.setGraphicSize(Std.int(bg.width * 0.5));
            bg.updateHitbox();
            add(bg);
		...
		scoreTxt.setFormat(Paths.font("funkin.ttf"), 34, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	public function startVideo(name:String):Void {
		...
		if(foundFile) {
			...
			(new FlxVideo(fileName)).finishCallback = function() {
				remove(bg);
				if(endingSong) {
					//if (isStoryMode && daSong == 'mad-buns') startVideo('MadBunsFinal');
					endSong();
				} else if(daSong == 'shine-stars') {
					startDialogue(dialogueJson);
				}
				else {
					startCountdown();
		...
		for (i in 0...4)
		{
			...
			if (player == 1)
				...
				else
				{
					...
					if (dad.curCharacter == 'mad-bunny')
						babyArrow = new StrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, i, player,'mad');
					else
						babyArrow = new StrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, i, player,'normal');
				}
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['weekjnz_nomiss', 'ur_bad',
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		notes.forEachAlive(function(note:Note) {
			...
			if (daNote.noteType == 'Dodge note' && daNote != note && daNote.mustPress && daNote.noteData == note.noteData && daNote.isSustainNote == note.isSustainNote && Math.abs(daNote.strumTime - note.strumTime) < 1){
				...
				switch(achievementName)
				{
					case 'weekjnz_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							{
								case 'week1':
									if(achievementName == 'weekjnz_nomiss') unlock = true;
							}
						}
					case 'ur_bad':
