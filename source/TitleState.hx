...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		gfDance.frames = Paths.getSparrowAtlas('gfDanceTitle');
			...
			gfDance.animation.addByPrefix('enter', 'gfEnter', 24);
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				if(gfDance != null) gfEnter == true;
				
				if(gfEnter == true){ 
					gfDance.animation.play('enter');
					gfDance.y = (titleJSON.gfy + titleJSON.gfEntery);
					gfDance.x = (titleJSON.gfx + titleJSON.gfEnterx);
				}
